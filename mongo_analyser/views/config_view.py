...
class ConfigView(Container):
    ...
    def _collect_settings_from_ui(self) -> Dict[str, Any]:
        ...
        try:
            ...
            explorer_sample_size_str = self.query_one("#config_explorer_sample_size_input",
                                                      Input).value
            try:
            ...
            self.app.notify(f"UI Error: Could not read setting for {e.widget}", severity="error",
                            title="Config Error")
        except Exception as e:
            logger.error(f"ConfigView: Unexpected error collecting settings from UI: {e}",
                         exc_info=True)
            self.app.notify("Unexpected error reading settings.", severity="error",
                            title="Config Error")
        return settings
    ...
    async def reset_configuration_button_pressed(self) -> None:
        ...
        confirm = await self.app.push_screen_wait(
            ConfirmDialog("Reset Configuration",
    ...
    def watch_config_save_feedback(self, new_feedback: Text) -> None:
        if self.is_mounted:
            try:
                ...
                logger.error(f"Error in watch_config_save_feedback (ConfigView): {e}",
                             exc_info=True)
