...
# Additionally, we can filter by common suffixes in the list_models method
...
class GoogleChat(LLMChat):
    def __init__(self, model_name: str, api_key: str = None, **kwargs: Any):
        ...
        init_kwargs_for_super = {"api_key": self._effective_api_key, **kwargs}
        init_kwargs_for_super["_model_kwargs"] = self._model_kwargs
        init_kwargs_for_super["_generation_config_input"] = generation_config_input
        init_kwargs_for_super["_safety_settings_input"] = safety_settings_input
        ...
        try:
            ...
            for model_name in all_models_from_api:
                ...
                if any(suffix in model_name for suffix in GOOGLE_MODEL_SUFFIX_BLOCKLIST):
                    continue
                # Prefer "latest" versions if a non-latest version also exists
                # This is a simple heuristic; more complex logic might be needed for perfect handling
                if "-latest" not in model_name and f"{model_name}-latest" in all_models_from_api:
                    # If a latest version exists, skip the non-latest one.
                    # This is not perfect as "model-001" and "model-latest" might both be valid.
                    # A more robust approach might involve checking if "model-001" is older than "model-latest"
                    # or if "model-latest" is just an alias. For now, this simple check helps.
                    pass  # This logic is a bit tricky; for now, let's rely on explicit blocklist and suffix list.

                # Filter out vision models if we only want text/chat models (can be made configurable)
                # For now, keeping vision models as they can often be used for text too.
                # if "vision" in model_name:
                #     continue

                filtered_models.append(model_name)

            # Sort to make the list predictable, e.g., by name or by some preference
